...
task :send_homework => :environment do
	...
	@users.each do |user|
		...
		if user.sentHomwork == false
			puts "two"
		if !user.groups.where("group_day = ?", 0.hours.ago.strftime("%A").downcase).nil? && !user.groups.where("group_day = ?", 0.hours.ago.strftime("%A").downcase).order("end_time asc").where("extra_class = ?", false).last.nil?
			puts "three"
			if user.groups.where("group_day = ?", 0.hours.ago.strftime("%A").downcase).where("extra_class = ?", false).order("end_time ASC").last.homework_assigned == true || user.groups.where("group_day = ?", 0.hours.ago.strftime("%A").downcase).order("end_time ASC").where("extra_class = ?", false).last.homework_assigned == false
				puts "four"
